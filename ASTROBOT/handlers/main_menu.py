...
# Обработчик кнопки "Продолжить консультацию"
@router.message(F.text == "🔄 Продолжить консультацию")
async def continue_consultation(message: Message, state: FSMContext):
    """
    Обработчик кнопки "Продолжить консультацию".
    Проверяет данные пользователя и баланс, предлагает продолжить консультацию.
    """
    ...
    # Предлагаем продолжить консультацию
    await message.answer(
        "🔄 Продолжаем нашу консультацию.\n\n"
        "Я помню наш предыдущий разговор и готов продолжить. Вы можете продолжить обсуждение предыдущей темы "
        "или начать новую. Если у вас появились новые вопросы или темы для обсуждения, я готов помочь.\n\n"
        "Если вы хотите изменить тему консультации, вы можете выбрать 'Начать консультацию' в главном меню.",
...
async def show_balance(message: Message):
    ...
    await message.answer(
        f"💰 Ваш текущий баланс: {balance:.0f} кредитов\n\n"
        "При каждом сообщении наш бот использует в качестве оплаты КРЕДИТЫ. Один кредит соответствует одному рублю.\n\n"
        "В среднем, стоимость консультации составляет от 300 до 1000 кредитов.\n\n"
        "Бот списывает с вашего баланса кредиты для формирования сообщений. То есть чем больше общаетесь - тем больше кредитов Вы тратите.\n\n"
        "Если Вы не пользуетесь консультацией, то деньги остаются на балансе. И если Вам нужно узнавать ответ лишь на конкретный точечный вопрос, то стоимость услуги будет минимальная.\n\n"
        "Выберите действие:",
...
3.3. Стоимость услуг рассчитывается исходя из количества обработанных токенов и устанавливается в кредитах(1 кредит = 1 рубль).
...
async def show_contacts(message: Message):
    ...
    contacts_text = (
        ...
        "Telegram: @mcyril\n\n"
        "Будем рады помочь вам с любыми вопросами!"
    )
...
async def confirm_consultation(callback: CallbackQuery, state: FSMContext):
    ...
    if is_initial_analysis:
       # Если это первый анализ - запрашиваем полное определение типа личности
        expert_prompt = "Определи мой тип личности, используя простые термины. Кратко объясни 1-2 ключевые особенности, а затем сразу предложи помощь в улучшении отношений с другими людьми. Спроси, в каких отношениях пользователь хотел бы получить совет (семья, романтика, работа, друзья)? Это ПЕРВЫЙ анализ пользователя."
