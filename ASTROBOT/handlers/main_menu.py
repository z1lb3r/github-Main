...
async def start_consultation(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        f"Стоимость анализа: {MIN_REQUIRED_BALANCE:.0f} кредитов\n"
        f"Ваш текущий баланс: {balance:.0f} кредитов",
        reply_markup=get_consultation_confirmation_keyboard()
    )

# Новый обработчик кнопки "Продолжить консультацию"
...
async def continue_consultation(message: Message, state: FSMContext):
    ...
    # Проверяем, был ли показан тип личности ранее
    type_shown = data.get("type_shown", False)
    
    # Если в истории есть сообщения, значит тип личности уже был показан
    ...
    if not holos_response:
        ...
        # Получаем данные через API Holos
    ...
    # Промпт для приветственного сообщения при продолжении консультации
...
async def show_balance(message: Message):
    ...
    await message.answer(
        ...
        "Если у Вас возникли вопросы по оплате, то, пожалуйста, обратитесь в нашу службу поддержки в телеграм: @vlad_adsmaster\n\n"
        "Выберите действие:",
